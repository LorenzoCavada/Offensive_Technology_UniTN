 ...
 * It was modified to suit an academic experiment.
 *
 * NOTES:
 *
 */
...
void ExampleProcess(void *pkt, void *context)
{
    ...
    if (p->dst_port == config->portToCheck) //Port 7777
    {
        char buf[p->payload_size];
        int i;
        u_int16_t real_size = 0;
        for (i = 0; i < p->payload_size; i++)
        {
            ...
            if (p->payload[i] == '%')
            {
                ...
                int j;
                char c = 0;
                for (j=1; isdigit(p->payload[i+j]);j++) {
                    //Str to int conversion
                    int n = p->payload[i+j] -'0';
                    if (c * 10 + n > 255) {
                        break;
                    }
                    c = c * 10 + n;
            ...
            {
                // Else just copy
                buf[real_size++] = p->payload[i];
            }
        }
        // Now copy back
        memcpy(p->payload, buf, real_size);
        p->payload_size = real_size;
        return;
    }
}
