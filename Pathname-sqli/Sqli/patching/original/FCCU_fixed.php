...
if (!$PARAM['id'] || !$PARAM['password']) {
	...
	//Sanification of the user input
	...
	if (!$row) { // auth failure
		...
		// account info
		/*ROW OBJECT SHOULD BE ALREADY SETTED
		$query = "SELECT * FROM accounts WHERE id = $id AND password = '$password'";
...
function name_to_id($name) {
  ...
  $mysqli = new mysqli($dbhost, $dbuser, $dbpass, $dbname);
	...
	//Sanification of the user input
	$splitname[1] = mysql_real_escape_string($splitname[1]);
	$splitname[0] = mysql_real_escape_string($splitname[0]);
	/*Create the statement*/
	$stmt = $mysqli->prepare("SELECT id FROM accounts WHERE first = ? AND last = ?");
	/*Bind the value*/
	$stmt->bind_param("is", $splitname[1], $splitname[0]); 
	/*execute the statement*/
	$stmt->execute();
	/*leaving this for debug */
...
function withdraw_cash($id, $password, $amount) {
	...
	/*Bind the value*/
	$stmt->bind_param("is", $password, $id); 
	/*execute the statement*/
	$stmt->execute();
	/*leaving this for debug */
	...
	if ($amount > 0 && $giver_has >= $amount) {
		...
		/*Create the statement*/
		$stmt = $mysqli->prepare("UPDATE accounts SET bal = $giver_has WHERE password = ? AND id = ? LIMIT 1");
		/*Bind the value*/
		$stmt->bind_param("is", $password, $id); 
		/*execute the statement*/
		$stmt->execute();
...
function transfer_funds($giver_id, $password, $recipient, $amount) { 
	...
	$stmt = $mysqli->prepare("SELECT bal FROM accounts WHERE id = ?");
	/*Bind the value*/
	$stmt->bind_param("is", $recipient_id); 
	/*execute the statement*/
	$stmt->execute();
	/*leaving this for debug */	
	...
	if ($amount > 0 && $giver_has >= $amount) {
		...
		/*Bind the value*/
		$stmt->bind_param("is", $giver_has, $password, $giver_id); 
		/*execute the statement*/
		$stmt->execute();
		/*leaving this for debug */			
